...
class ScreenshotCandidateService:
    def __init__(self, storage: StorageService, prepared: PreparedVideoService, states: TeachingStateService,
                 config: ScreenshotCandidateConfig | None = None) -> None:
    ...
    def get_summary(self, video_id: str) -> dict | None:
        ...
        try:
            ...
            return None
        return summary if actual_images == kept_count else None
    ...
    @staticmethod
    def _seek_frame(capture: cv2.VideoCapture, frame_index: int, timestamp_seconds: float) -> np.ndarray:
        ...
        if ok and frame is not None:
            return frame
        raise AppError(ErrorCode.SCREENSHOT_EXTRACTION_FAILED, "A full-resolution checkpoint frame could not be extracted.", 422)
    ...
    def scan(self, video_id: str, progress: ProgressCallback) -> dict:
        ...
        try:
            for candidate_index, state in enumerate(state_records):
                frame_index, timestamp = int(state["frame_index"]), float(state["timestamp_seconds"])
                frame = self._seek_frame(capture, frame_index, timestamp)
                ...
                if keep:
                    ...
                    duplicate_count += 1
                payloads.append({
                    "candidate_index": candidate_index, "checkpoint_index": int(state["checkpoint_index"]),
                    ...
                    "protected": protected, "kept": keep, "image_filename": filename,
                    ...
                    "edge_density": round(edge_density, 6), "contrast_std": round(contrast_std, 6),
                })
                progress(min(96.0, (candidate_index + 1) / expected_count * 96.0), f"Seeking full-resolution checkpoints... {candidate_index + 1:,}/{expected_count:,}")
            ...
            stat = prepared.local_video_path.stat()
            summary = {
                "video_id": video_id, "status": "READY", "pipeline_version": PIPELINE_VERSION,
                "source_checkpoint_count": expected_count, "kept_candidate_count": kept_count,
                "duplicate_candidate_count": duplicate_count, "protected_kept_count": protected_kept_count,
                "content_loss_risk_count": risk_count, "deduplication_conservative": True,
                "extraction_mode": "RANDOM_ACCESS_CHECKPOINTS", "filter_config": self.config.to_dict(),
                "generated_at": utc_now_iso(), "states_generated_at": states["generated_at"],
