...
class TeachingStateService:
    ...
    def _records(self, video_id: str, expected_pairs: int, progress: ProgressCallback) -> Iterator[dict]:
        ...
        if not path.exists():
            raise AppError(ErrorCode.CHANGE_ANALYSIS_NOT_FOUND, "Adaptive visual evidence is missing.", 409)
        seen = 0
        last_timestamp = -1.0
        last_frame = -1
        update_every = max(10, expected_pairs // 100) if expected_pairs else 10
        try:
            with path.open("r", encoding="utf-8") as handle:
                for line in handle:
                    ...
                    try:
                        ...
                        raise AppError(ErrorCode.TEACHING_STATE_FAILED, "Adaptive visual evidence is invalid.", 422) from exc
                    if frame_index <= previous_index or frame_index <= last_frame:
                        raise AppError(ErrorCode.FRAME_SEQUENCE_MISMATCH, "Adaptive visual evidence contains frame reordering.", 422)
                    if previous_timestamp < last_timestamp or timestamp < previous_timestamp:
                        raise AppError(ErrorCode.FRAME_SEQUENCE_MISMATCH, "Adaptive visual evidence contains non-monotonic timestamps.", 422)
                    if kind not in {"NONE", "LOCAL", "STRUCTURAL", "SCENE"}:
                        raise AppError(ErrorCode.TEACHING_STATE_FAILED, "Adaptive visual evidence contains an unknown change type.", 422)
                    seen += 1
                    last_timestamp = timestamp
                    last_frame = frame_index
                    if seen % update_every == 0:
                        pct = seen / max(1, expected_pairs) * 100.0
                        progress(min(99.0, pct), f"Detecting stable teaching states... {seen:,}/{expected_pairs:,} adaptive transitions")
                    yield record
        except AppError:
            raise
        except OSError as exc:
            raise AppError(ErrorCode.TEACHING_STATE_FAILED, "Adaptive visual evidence could not be read.", 500) from exc
        if seen != expected_pairs:
            raise AppError(ErrorCode.FRAME_SEQUENCE_MISMATCH, "Teaching-state detection did not inspect all adaptive transitions.", 422)
    ...
    def scan(self, video_id: str, progress: ProgressCallback) -> dict:
        ...
        if not timeline:
            raise AppError(ErrorCode.TIMELINE_NOT_FOUND, "Build frame metadata before teaching-state detection.", 409)
        if not changes:
            raise AppError(ErrorCode.CHANGE_ANALYSIS_NOT_FOUND, "Run adaptive visual analysis before teaching-state detection.", 409)
        ...
        if frame_count <= 0 or expected_pairs <= 0:
            raise AppError(ErrorCode.FRAME_SEQUENCE_MISMATCH, "Adaptive visual evidence is empty or invalid.", 422)
        ...
        try:
                ...
                for checkpoint in self.detector.detect(self._records(video_id, expected_pairs, progress)):
                    if checkpoint.frame_index >= frame_count:
                        checkpoint = TeachingCheckpoint(
                            ...
                            timestamp_seconds=min(float(timeline.get("duration_seconds") or checkpoint.timestamp_seconds), checkpoint.timestamp_seconds),
                            reason=checkpoint.reason,
                            stability_seconds=checkpoint.stability_seconds,
                            source_change_kind=checkpoint.source_change_kind,
                            max_change_score_since_previous=checkpoint.max_change_score_since_previous,
                            activity_pair_count_since_previous=checkpoint.activity_pair_count_since_previous,
                            protected_before_transition=checkpoint.protected_before_transition,
                        )
                    write_checkpoint(handle, checkpoint)
                if checkpoint_count == 0:
                    write_checkpoint(handle, TeachingCheckpoint(
                        frame_index=max(0, frame_count - 1),
                        timestamp_seconds=float(timeline.get("last_timestamp_seconds") or 0.0),
            ...
            progress(100.0, f"Stable teaching states ready. {checkpoint_count:,} checkpoint candidates found from adaptive evidence.")
            return summary
        except AppError:
            temp_path.unlink(missing_ok=True)
            raise
