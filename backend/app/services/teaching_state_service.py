...
class TeachingStateService:
    def __init__(self, storage: StorageService, prepared: PreparedVideoService, timeline: FrameTimelineService,
                 changes: VisualChangeService, detector: TeachingStateDetector | None = None) -> None:
    ...
    def _records(self, video_id: str, expected_pairs: int, progress: ProgressCallback) -> Iterator[dict]:
        ...
        seen, last_frame = 0, -1
        last_timestamp = -1.0
        update_every = max(20, expected_pairs // 150) if expected_pairs else 20
        try:
            with path.open("r", encoding="utf-8") as handle:
                for line in handle:
                    ...
                    try:
                        ...
                        raise AppError(ErrorCode.TEACHING_STATE_FAILED, "The adaptive visual change map is invalid.", 422) from exc
                    if previous_index < 0 or frame_index <= previous_index or frame_index <= last_frame:
                        raise AppError(ErrorCode.FRAME_SEQUENCE_MISMATCH, "The adaptive change map contains reordered samples.", 422)
                    if previous_timestamp < 0 or timestamp <= previous_timestamp or timestamp <= last_timestamp:
                        raise AppError(ErrorCode.FRAME_SEQUENCE_MISMATCH, "The adaptive change map contains non-monotonic timestamps.", 422)
                    if kind not in {"NONE", "LOCAL", "STRUCTURAL", "SCENE"}:
                        raise AppError(ErrorCode.TEACHING_STATE_FAILED, "The adaptive change map contains an unknown change type.", 422)
                    seen += 1
                    last_frame, last_timestamp = frame_index, timestamp
                    if seen % update_every == 0:
                        pct = seen / expected_pairs * 100.0 if expected_pairs else 100.0
                        progress(min(99.0, pct), f"Streaming stable-state detection... {seen:,}/{expected_pairs:,} sampled transitions")
                    yield record
        except AppError:
            raise
        except OSError as exc:
            raise AppError(ErrorCode.TEACHING_STATE_FAILED, "The adaptive visual change map could not be read.", 500) from exc
        if seen != expected_pairs:
            raise AppError(ErrorCode.FRAME_SEQUENCE_MISMATCH, "Stable-state detection did not inspect every sampled visual transition.", 422)
    ...
    def scan(self, video_id: str, progress: ProgressCallback) -> dict:
        ...
        if not timeline:
            raise AppError(ErrorCode.TIMELINE_NOT_FOUND, "Build the source timeline before teaching-state detection.", 409)
        if not changes:
        ...
        if frame_count <= 0:
            raise AppError(ErrorCode.FRAME_SEQUENCE_MISMATCH, "Source frame metadata is invalid.", 422)
        if frame_count > 1 and expected_pairs <= 0:
            raise AppError(ErrorCode.FRAME_SEQUENCE_MISMATCH, "Adaptive visual analysis contains no sampled transitions.", 422)
        ...
        checkpoint_count = 0
        first_checkpoint_timestamp = None
        ...
        def write_checkpoint(handle, checkpoint: TeachingCheckpoint) -> None:
            ...
            handle.write(json.dumps(checkpoint.to_dict(checkpoint_count), separators=(",", ":")) + "\n")
        ...
        try:
            with temp_path.open("w", encoding="utf-8") as handle:
                if frame_count == 1:
                    write_checkpoint(handle, TeachingCheckpoint(frame_index=0,
                        timestamp_seconds=float(timeline.get("first_timestamp_seconds") or 0.0),
                        reason=TeachingStateReason.SINGLE_FRAME, stability_seconds=0.0, source_change_kind="NONE",
                        max_change_score_since_previous=0.0, activity_pair_count_since_previous=0,
                    ...
                    for checkpoint in self.detector.detect(self._records(video_id, expected_pairs, progress)):
                        if checkpoint.frame_index >= frame_count:
                            raise AppError(ErrorCode.TEACHING_STATE_FAILED, "A teaching checkpoint references a frame outside the source video.", 422)
            ...
            stat = prepared.local_video_path.stat()
            summary = {
                "video_id": video_id, "status": "READY", "checkpoint_count": checkpoint_count,
                ...
                "coverage_complete": True, "adaptive_sampling": bool(changes.get("adaptive_sampling")),
                "source_pair_count": expected_pairs, "detector_config": self.detector.config.to_dict(),
                "generated_at": utc_now_iso(), "timeline_generated_at": timeline["generated_at"],
                "changes_generated_at": changes["generated_at"], "source_size_bytes": stat.st_size,
