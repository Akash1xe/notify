...
class TranscriptionService:
    def __init__(
        ...
        self._raw_transcript_lock = threading.Lock()
    ...
    def _load_faster_whisper_model(model_name: str, device: str, compute_type: str) -> Any:
        try:
            ...
            raise AppError(ErrorCode.WHISPER_UNAVAILABLE, "faster-whisper is not installed. Install the backend requirements and try again.", 503) from exc
        try:
            return WhisperModel(model_name, device=device, compute_type=compute_type)
        except Exception as exc:
            raise AppError(ErrorCode.WHISPER_UNAVAILABLE, "The configured Whisper model could not be loaded. The first local run may need to download the model.", 503) from exc
    ...
    def _valid_transcript_summary(self, video_id: str) -> dict | None:
        ...
        if summary.get("model_name") != self.model_name or summary.get("requested_language") != self.language:
            return None
        if summary.get("device") not in {None, self.device} or summary.get("compute_type") not in {None, self.compute_type}:
            return None
        return summary
    ...
    def _transcribe(self, video_id: str, progress: ProgressCallback) -> dict:
        ...
        try:
            segments_iter, info = model.transcribe(str(audio_path), language=self.language or None, beam_size=5, vad_filter=True, word_timestamps=False)
        except AppError:
            raise
        except Exception as exc:
        ...
        try:
            for index, segment in enumerate(segments_iter):
                ...
                if not text:
                    continue
                records.append({"segment_index": len(records), "start_seconds": round(start, 6), "end_seconds": round(end, 6), "text": text})
        ...
        self._write_jsonl_atomic(self.storage.transcript_segments_path(video_id), records, "Timestamped transcript segments could not be saved.")
        stat = prepared.local_video_path.stat()
        ...
        try:
            ...
            probability = 0.0
        summary = {
            "video_id": video_id, "status": "READY", "model_name": self.model_name,
            "requested_language": self.language, "device": self.device, "compute_type": self.compute_type,
            "detected_language": detected_language, "language_probability": round(probability, 6),
            "duration_seconds": round(source_duration, 6), "segment_count": len(records), "word_count": word_count,
            "audio_sample_rate_hz": 16000, "source_size_bytes": stat.st_size, "source_mtime_ns": stat.st_mtime_ns,
    ...
    def ensure_raw_transcript(self, video_id: str, progress: ProgressCallback | None = None) -> dict:
        callback = progress or (lambda _stage, _pct, _message: None)
        existing = self._valid_transcript_summary(video_id)
        if existing:
            callback("TRANSCRIBE", 86.0, "Reusing the existing timestamped transcript.")
            return existing
        with self._raw_transcript_lock:
            existing = self._valid_transcript_summary(video_id)
            if existing:
                callback("TRANSCRIBE", 86.0, "Reusing the timestamped transcript produced by a parallel task.")
                return existing
            return self._transcribe(video_id, callback)
    ...
    def _align(self, video_id: str, transcript: dict, trusted: dict, progress: ProgressCallback) -> dict:
        ...
        for screenshot in trusted_records:
            ...
            nearby = [segment for segment in segments if float(segment["end_seconds"]) >= window_start and float(segment["start_seconds"]) <= window_end]
            text = " ".join(str(segment["text"]).strip() for segment in nearby if str(segment["text"]).strip()).strip()
            if text:
                aligned_count += 1
            mappings.append({
                "trusted_index": int(screenshot["trusted_index"]), "candidate_index": int(screenshot["candidate_index"]),
                "frame_index": int(screenshot["frame_index"]), "timestamp_seconds": timestamp,
                "context_start_seconds": round(window_start, 6), "context_end_seconds": round(window_end, 6),
                "segment_indexes": [int(segment["segment_index"]) for segment in nearby], "text": text,
            })
        self._write_jsonl_atomic(self.storage.screenshot_transcript_map_path(video_id), mappings, "Screenshot-to-transcript alignment could not be saved.")
        summary = {
            "video_id": video_id, "status": "READY", "trusted_screenshot_count": len(trusted_records),
            "aligned_screenshot_count": aligned_count, "unaligned_screenshot_count": len(trusted_records) - aligned_count,
            "context_before_seconds": 6.0, "context_after_seconds": 8.0,
            "transcript_generated_at": transcript["generated_at"], "trusted_generated_at": trusted["generated_at"],
            "generated_at": utc_now_iso(),
        }
    ...
    def process(self, video_id: str, progress: ProgressCallback) -> dict:
        trusted = self._trusted_summary(video_id)
        transcript = self.ensure_raw_transcript(video_id, progress)
        alignment = self._valid_alignment_summary(video_id, transcript, trusted)
        if not alignment:
