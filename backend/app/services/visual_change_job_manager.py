...
class VisualChangeJobManager:
    def __init__(self, storage: StorageService, changes: VisualChangeService) -> None:
    ...
    def start(self, video_id: str) -> tuple[JobRecord, bool]:
        ...
        if existing:
            job = self._new_job(video_id, JobStatus.READY, 100.0, "Visual change analysis already exists and is valid.")
            with self._lock:
        ...
        with self._lock:
            ...
            job = self._new_job(video_id, JobStatus.QUEUED, 0.0, "Queued for visual frame comparison...")
            self._jobs[job.job_id] = job
    ...
    def _run(self, job_id: str) -> None:
        job = self._jobs[job_id]
        try:
            self._update(job_id, JobStatus.COMPARING_FRAMES, 0.0, "Opening frame timeline and prepared lecture...")
            self.changes.scan(
                job.video_id,
                lambda progress, message: self._update(job_id, JobStatus.COMPARING_FRAMES, progress, message),
            )
            self._update(job_id, JobStatus.READY, 100.0, "Visual change map ready.")
            self.storage.cleanup_job_workspace(job_id)
        except AppError as exc:
            logger.warning("Visual change analysis failed for job %s: %s", job_id, exc.code)
            with self._lock:
            ...
            logger.exception("Unexpected visual change analysis failure for job %s", job_id)
            with self._lock:
                current = self._jobs[job_id]
                current.status = JobStatus.FAILED
                current.message = "Visual change analysis failed unexpectedly."
                current.error_code = ErrorCode.INTERNAL_ERROR
                current.error_message = "Visual change analysis failed unexpectedly."
                current.updated_at = utc_now_iso()
                self._persist(current)
        finally:
            with self._lock:
    ...
    def get(self, job_id: str) -> JobRecord:
        ...
        if persisted and persisted.job_type == JobType.VISUAL_CHANGE:
            return persisted
        raise AppError(ErrorCode.JOB_NOT_FOUND, "The requested visual change analysis job was not found.", 404)
