...
export const api = {
  ...
  startOcrEnrichment: (videoId: string) => apiFetch<StartOcrEnrichmentResponse>("/api/analysis/ocr/start", { method: "POST", body: JSON.stringify({ video_id: videoId }) }, 30_000),
  getOcrJob: (jobId: string) => apiFetch<AnalysisJobResponse>(`/api/analysis/ocr/jobs/${encodeURIComponent(jobId)}`, undefined, 10_000),
  getOcrResult: (videoId: string) => apiFetch<OcrResultResponse>(`/api/analysis/${encodeURIComponent(videoId)}/ocr`, undefined, 10_000),
